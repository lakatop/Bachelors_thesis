...
Ďalšie vlastnosti aplikácie sú napríklad parsovanie HID Report Descriptoru vďaka ktorému bude v budúcnosti jednoduchšie pridať sémantickú analýzu rôznym ďalším HID zariadeniam. Celkový návrh aplikácie by mal ponúknuť možnosť budúcej implementácie ďalších USB tried pre~prípadné rozšírenie.
}
\def\AbstraktEN{%
USB is the most common way of connecting peripherals to the PC. The goal of this thesis is to create a software application which analyzes communication between a device connected to this bus and a computer.

An application should be capable to reasonably show analyzed data. Its specific focus is on HID class devices. This also brings semantic analysis of given subset of HID devices, which consists of mouse, keyboard and joystick. The methods that application use to visually represent data were created upon an inspiration from already existing ones, whereas it combines and impoves their capabilities to achieve the best possible result.

Another feature that application has is for example ability to parse HID Report Descriptor, to accomplish easier addition of new HID devices for semantic analysis. Overall design of application should be general enough so it will be possible to add analysis for other USB classes.
}
